...
export const ReportTaskLandScreen = ({ route, navigation }) => {
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      if (Number(quality) <= 0 || Number(quality) >= 100) {
        Toast.show({
          type: "error",
          text1: "Số lượng phải lớn hơn 1 và nhỏ hơn 100",
        });
        return;
      }

      console.log("note: " + Number(quality) / 100);
      let reportData = {
        content: note,
        quality_report: Number(quality) / 100,
        url: [],
      };
        ...
        <View style={styles.container}>
          <View style={styles.rowContainer}>
            ...
            <Text style={styles.content}>
              {taskInfo?.request?.type === "report_land"
                ? "Báo cáo tình trạng đất"
                : "Hỗ trợ kĩ thuật"}
            </Text>
          </View>
          <View style={styles.rowContainer}>
            <Text style={styles.title}>Chất lượng </Text>
            <TextInput
              value={quality}
              ...
              maxLength={3}
              placeholder="Chất lượng đất từ 1 đến 100"
              inputMode="decimal"
            />
          </View>
          <View style={styles.rowContainer}>
