...
const PlanToStandardFarmingInput = forwardRef(
  ({ plantStages, setPlantStages }, ref) => {
    ...
    const removeStage = (id) => {
      setPlantStages(plantStages.filter((stage) => stage.id !== id));
    };

    const addInputBlock = (stageId) => {
      setPlantStages((prevStages) =>
    ...
    const removeInputBlock = (stageId, index) => {
      const lengthOfThisStage = plantStages.filter(
    ...
    const handleCheckValidateDate = () => {
      ...
      if (plantStages[0].inputs[0].from != 1) {
        Toast.show({
          type: "error",
      ...
      plantStages.map((stage, idxStage) => {
        if (!isFalse) {
          if (!stage.title.trim()) {
            Toast.show({
              type: "error",
      ...
      plantStages.map((stage, idxStage) => {
        if (!isFalse) {
          ...
          if (idxStage > 0) {
            const lastToPrev =
              plantStages[idxStage - 1].inputs[
                plantStages[idxStage - 1].inputs.length - 1
              ].to;
    ...
    return (
      <View>
        ...
        {plantStages &&
          ...
          plantStages.map((stage, indexStage) => (
            <View key={stage.id} style={styles.inputStageContainer}>
              <View style={styles.inputRow}>
                ...
                />
                {indexStage === plantStages.length - 1 && (
                  <TouchableOpacity
              ...
              {stage.materials.map((material, materialIndex) => (
                <View key={materialIndex} style={styles.inputDateContainer}>
                  <View style={styles.inputMaterialContainer}>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        removeMaterialBlock(stage.id, materialIndex)
                      }
                    >
                      <MaterialCommunityIcons
                        name="delete-outline"
